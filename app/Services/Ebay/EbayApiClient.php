...
class EbayApiClient
{
    ...
    /**
     * Get an application-level (client_credentials) access token.
     *
     * Unlike user tokens, this is not tied to a SalesChannel/seller — it
     * authenticates the app itself for public data APIs (e.g. Marketplace
     * Insights, Browse). Cached until shortly before expiry (~2h).
     */
    public function getApplicationToken(): string
    {
        return Cache::remember('ebay_application_token', now()->addMinutes(110), function () {
            ...
            if ($response->failed()) {
                ...
                throw new Exception('eBay application token request failed: ' . $response->body());
            }
