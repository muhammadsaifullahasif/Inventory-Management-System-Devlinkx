                        ...
                        @foreach ($permissions as $permission)
                            <div class="col-md-3 mb-2">
                                {{-- <div class="form-check">
                                    <input type="checkbox" id="perm-{{ $permission->id }}" value="{{ $permission->name }}" name="permission[]" class="form-check-input" {{ $hasPermissions->contains($permission->name) ? 'checked' : '' }}>
                                    <label for="perm-{{ $permission->id }}" class="form-check-label">{{ $permission->name }}</label>
                                </div> --}}
                                <div class="item-checkbox ms-1">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" id="perm-{{ $permission->id }}" value="{{ $permission->name }}" name="permission[]" class="custom-control-input checkbox" {{  $hasPermissions->contains($permission->name) ? 'checked' : '' }}>
