            ...
            <form action="{{ route('permissions.update', $permission->id) }}" method="post">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $permission->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Permission Name (e.g., view products)" required>
                        @error('name')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="category" name="category" class="form-select @error('category') is-invalid @enderror">
                            ...
                            @foreach($categories as $key => $label)
                                <option value="{{ $key }}" {{ old('category', $permission->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('category')
                            <span class="invalid-feedback">{{ $message }}</span>
