    ...
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center">
            <div class="page-header-title">
                <h5 class="m-b-10">Comparison Report</h5>
            </div>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('reports.index') }}">Reports</a></li>
                <li class="breadcrumb-item">Comparison Report</li>
            </ul>
        </div>
...
@section('content')
        ...
        <div class="card mb-4">
            ...
            <div class="collapse show" id="filterCollapse">
                <div class="card-body py-3">
                    <form action="{{ route('reports.comparison') }}" method="GET">
                        <div class="row g-3">
                            <div class="col-md-2">
                                    ...
                                    <div class="dropdown-menu p-2" id="categoryDropdownMenu" style="max-height: 250px; overflow-y: auto; min-width: 220px;">
                                        @forelse($categories as $category)
                                            <div class="form-check">
                                                <input class="form-check-input category-filter-checkbox" type="checkbox" name="category_id[]" value="{{ $category->id }}" id="comparison_report_category_{{ $category->id }}"
                                                    {{ in_array($category->id, $categoryIds) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="comparison_report_category_{{ $category->id }}">
                                                    {{ $category->name }}
                                                </label>
                                            </div>
                                        @empty
                        ...
                        <div class="row mt-3">
                            <div class="col-12 d-flex align-items-end gap-2">
                                ...
                                <a href="{{ route('reports.comparison') }}" class="btn btn-light-brand btn-sm">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    ...
                    COGS vs Profit by {{ ucfirst($groupBy) }}
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $groupBy === 'product' ? 8 : ($groupBy === 'order' ? 8 : 6) }}" class="text-center py-5 text-muted">
                                        ...
                                        <p class="mt-3">No comparison data found for the selected period.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div class="col-12 mt-4">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0"><i class="feather-info me-2"></i>Report Summary</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm">
                            ...
                            <tr>
                                <td>Total COGS:</td>
                                <td class="text-end fw-semibold text-danger">{{ number_format($summary['total_cogs'], 2) }}</td>
                            </tr>
                            <tr class="table-light">
                                <td class="fw-bold">Gross Profit:</td>
                                <td class="text-end fw-bold {{ $summary['gross_profit'] >= 0 ? 'text-success' : 'text-danger' }}">
                                    {{ number_format($summary['gross_profit'], 2) }}
                                </td>
                            </tr>
                            <tr>
                                <td class="fw-bold">Gross Margin:</td>
                                <td class="text-end fw-bold">{{ number_format($summary['gross_margin'], 2) }}%</td>
                            </tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm">
                            ...
                            <tr>
                                <td>Total Revenue:</td>
                                <td class="text-end fw-semibold text-success">{{ number_format($paidSummary['total_revenue'], 2) }}</td>
                            </tr>
                            <tr>
                                <td>Total COGS:</td>
                                <td class="text-end fw-semibold text-danger">{{ number_format($paidSummary['total_cogs'], 2) }}</td>
                            </tr>
                            <tr class="table-light">
                                <td class="fw-bold">Gross Profit:</td>
                                <td class="text-end fw-bold {{ $paidSummary['gross_profit'] >= 0 ? 'text-success' : 'text-danger' }}">
                                    {{ number_format($paidSummary['gross_profit'], 2) }}
                                </td>
                            </tr>
                            <tr>
                                <td class="fw-bold">Gross Margin:</td>
                                <td class="text-end fw-bold">{{ number_format($paidSummary['gross_margin'], 2) }}%</td>
                            </tr>
                        </table>
                    </div>
                </div>
