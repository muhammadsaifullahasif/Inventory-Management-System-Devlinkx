...
@section('header')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 d-inline mr-2">Warehouse Edit</h1>
    ...
    <div class="card card-body">
        <form action="{{ route('warehouses.update', $warehouse->id) }}" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $warehouse->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Warehouse Name">
                @error('name')
            ...
            <div class="mb-3">
                <div class="custom-control custom-switch">
                    <input type="checkbox" id="is_default" value="1" name="is_default" class="custom-control-input" {{ (old('is_default', $warehouse->is_default) == 1) ? 'checked' : '' }}>
                    <label for="is_default" class="custom-control-label">Is Default</label>
                </div>
            </div>
            <div class="mb-3">
                <label for="racks">No of Racks:</label>
                <input type="text" id="racks" name="racks" value="{{ old('racks', $warehouse->racks->count()) }}" class="form-control form-control-sm" placeholder="Number of Racks in Warehouse">
                @error('racks')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            <div id="racksCountContainer" class="table-responsive">
                <table class="table table-striped table-hover table-s">
                    <thead>
                        <tr>
